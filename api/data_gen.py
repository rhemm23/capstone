...
for image in os.listdir('/home/rh/thumbnails128x128'):
  images.append(os.path.join('/home/rh/thumbnails128x128', image))
...
with mp_det.FaceDetection() as det:
  for image in images:
    ...
    if not results:
      ...
      reye = [
        int(results[0].location_data.relative_keypoints[0].x * w),
        int(results[0].location_data.relative_keypoints[0].y * h)
      ]
      leye = [
        int(results[0].location_data.relative_keypoints[1].x * w),
        int(results[0].location_data.relative_keypoints[1].y * h)
      ]
    x0 = int(face.xmin * w)
    y0 = int(face.ymin * h)
